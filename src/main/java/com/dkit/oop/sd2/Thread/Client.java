...
public class Client {
    ...
    public void start() {
        ...
        try {
            Socket socket = new Socket("localhost", 2626); // connect to server socket
    ...
    private static void receiveFile(String fileName)
        ...
        System.out.print("Server:  Bytes remaining to be read from socket: ");
        ...
        while (size > 0 &&
            ...
            // above, we read a number of bytes from stream to fill the buffer (if there are
            // enough remaining)
            // - the number of bytes we must read is the smallest (min) of: the buffer
            // length and the remaining size of the file
            // - (remember that the last chunk of data read will usually not fill the
            // buffer)
            ...
            // 'size' represents the number of bytes remaining to be read from the socket
            // stream.
            // We repeat this until all the bytes are dealt with and the size is reduced to
            // zero
            size = size - bytes;
...
// LocalTime time = LocalTime.parse(timeString); // Parse timeString -> convert
// to LocalTime object if required